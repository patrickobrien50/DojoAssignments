...
class GameScene: SKScene, SKPhysicsContactDelegate {
    ...
    override func didMove(to view: SKView) {
        backgroundColor = UIColor(red: 100.0/255.0, green: 170.0/255.0, blue: 255.0/255.0, alpha: 1.0)
        
        swipedUp.direction = .up
        view.addGestureRecognizer(swipedUp)
        if isStarted {
            hero.jump(sender: swipedUp)
        }
...
//
    ...
    func restart() {
        ...
        func gameOverSound() {
            guard let url = Bundle.main.url(forResource: "Game Over sound", withExtension:".m4a") else {
                print("url not found")
                return
            }
